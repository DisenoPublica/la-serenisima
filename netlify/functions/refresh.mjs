// Netlify Scheduled Function · todos los días 13:00 UTC = 10:00 Argentina
import { getStore } from '@netlify/blobs';

export const config = { schedule: '0 13 * * *' };

const SEARCH_ID = Netlify.env.get('MELTWATER_SEARCH_ID') || '29139916';
const DAYS = 31;
const AXES = {
 "s1": [
  "Arcor",
...
async function fetchAll(start, end) {
  ...
  for (let page = 1; page <= 20; page++) {
    const r = await fetch(`https://api.meltwater.com/v3/search/${SEARCH_ID}`, {
      method: 'POST',
      headers: { apikey: Netlify.env.get('MELTWATER_API_KEY'), 'Content-Type': 'application/json', Accept: 'application/json' },
...
export default async () => {
  ...
  return new Response(JSON.stringify({ ok: true, docs: docs.length }), { headers: { 'content-type': 'application/json' } });
};
